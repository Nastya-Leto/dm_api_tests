...
class TestChangeEmail:
    def test_successful_change_email_user(self, account_helper_f, prepare_user):
        ...
        account_helper_f.change_email_user(login, password, new_email)
        ...
        token = account_helper_f.get_new_activation_token(new_email=new_email)
        account_helper_f.activation_user(token=token)
        response = account_helper_f.user_login(login, password)
        assert response.status_code == 200, f'Пользователь не был авторизован{response.text}'


    def test_unsuccessful_change_email_user(self, account_helper_f, prepare_user):
        ...
        account_helper_f.register_new_user(login, password, email)
        account_helper_f.user_login(login, password)
        response = account_helper_f.change_email_user(login, password, new_email)
