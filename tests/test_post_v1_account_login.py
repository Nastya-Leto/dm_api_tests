...
class TestLoginUser:
    def test_successful_login(self, account_helper_f, prepare_user):
    ...
    def test_unsuccessful_login(self, account_helper_f, prepare_user):
        login = prepare_user.login
        password = prepare_user.password
        email = prepare_user.email

        response = account_helper_f.register_new_user(login, password, email, with_activate=False)
